...
do
cat > $SCRIPTS/mit_${SAMPLE_ID}.sh <<EOF
samtools idxstats $ALIGNMENTFILES/${SAMPLE_ID}_dedup.bam  | cut -f 1 | grep -v MT | xargs samtools view -b -@ 30 $ALIGNMENTFILES/${SAMPLE_ID}_dedup.bam > $ALIGNMENTFILES/${SAMPLE_ID}_mtfiltered.bam
EOF

bash $SCRIPTS/mit_${SAMPLE_ID}.sh
