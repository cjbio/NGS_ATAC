...
do
cat > $SCRIPTS/index_${SAMPLE_ID}.sh <<EOF
samtools index $ALIGNMENTFILES/${SAMPLE_ID}_dedup.bam $ALIGNMENTFILES/${SAMPLE_ID}_dedup.bam.bai
EOF

bash $SCRIPTS/index_${SAMPLE_ID}.sh
